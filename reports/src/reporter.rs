...
pub fn generate_flame_report(prefix: &str) {
    println!("hi flame");
    ...
    {
        let filespec = format!("{report_dir}/{prefix}-flame.html");
        let file = &mut std::fs::File::create(filespec).unwrap();
        flame::dump_html(file).unwrap_or_else(|why| { println!("! {:?}", why.kind()); });
    }

    {
        let filespec = format!("{report_dir}/{prefix}-flame-merge.html");
        let file = &mut std::fs::File::create(filespec).unwrap();

        let mut spans = flame::threads().into_iter().next().unwrap().spans;
        merge_spans(&mut spans);
        dump_html_custom(file, &spans).unwrap_or_else(|why| { println!("! {:?}", why.kind()); });
    }
}
