    ...
    RationalSumcheckProver<B, E, H>
{
    ...
    pub fn generate_proof(
        ...
        let f_hat_evals: Vec<B> = self
            .summing_domain
            .iter()
            .map(|x| {
                polynom::eval(&self.numerator_coeffs, *x)
                    / polynom::eval(&self.denominator_coeffs, *x)
            })
            .collect();
        ...
        let p_eval_domain_evals =
            polynom::eval_many(&self.numerator_coeffs, &self.evaluation_domain);
        let q_eval_domain_evals =
            polynom::eval_many(&self.denominator_coeffs, &self.evaluation_domain);
