    ...
    > LincheckProver<'a, B, E, H>
{
    ...
    pub fn generate_t_alpha(&self, t_evals: Vec<B>) -> Vec<B> {
        let mut t_alpha_eval_domain_poly: Vec<B> =
        ...
        polynom::interpolate(
            &self.options.evaluation_domain.to_vec(),
            &t_evals.to_vec(),
            true,
        )
    }
    ...
    pub fn generate_poly_prod(&self, t_alpha_coeffs: &Vec<B>) -> Vec<B> {
        ...
        let mut alpha_to_h_size = self.alpha.exp(B::PositiveInteger::from(
        ...
        let u_alpha_evals: Vec<B> = self
            .options
            .evaluation_domain
            .iter()
            .map(|e| polynom::eval(&u_numerator, *e) / polynom::eval(&u_denominator, *e))
            .collect();
        let u_alpha_coeffs2 =
    ...
    pub fn generate_poly_prod_evals(&self, t_alpha: &Vec<B>) -> Vec<B> {
        ...
        // 2. Polynom includes a mul and a sub function, use these to do the respective ops
        // botttom of page 29
        let alpha_to_h_size = self.alpha.exp(B::PositiveInteger::from(
            self.options.size_subgroup_h as u64,
        ));
        let mut u_numerator = vec![B::ZERO; (self.options.size_subgroup_h).try_into().unwrap()];
        u_numerator[0] = alpha_to_h_size.neg();
